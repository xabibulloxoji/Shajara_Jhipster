...
type RestOf<T extends IPerson | NewPerson> = Omit<T, 'born' | 'death'> & {
  born?: string | null;
  death?: string | null;
};
...
export class PersonService {
  ...
  protected convertDateFromClient<T extends IPerson | NewPerson | PartialUpdatePerson>(person: T): RestOf<T> {
    return {
      ...person,
      born: person.born?.toJSON() ?? null,
      death: person.death?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPerson: RestPerson): IPerson {
    return {
      ...restPerson,
      born: restPerson.born ? dayjs(restPerson.born) : undefined,
      death: restPerson.death ? dayjs(restPerson.death) : undefined,
    };
  }
